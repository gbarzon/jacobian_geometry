...
def fname_mean(dynamics,num_nodes,pp):
    return "../Results_"+str(dynamics)+"/ER_MeanDist_N%d_p%g.dat" % (num_nodes, pp)

def fname_mean_theor(dynamics,num_nodes,pp):
    return "../Results_"+str(dynamics)+"/ER_MeanDist_N%d_p%g_Theoretical.dat" % (num_nodes, pp)


def fname_dij(dynamics,num_nodes, pp, time):
    return "../Results_"+str(dynamics)+"/ER_Dij_N%d_p%g_T%s.dat" % (num_nodes, pp, str('{:.6f}'.format(time)))

def fname_network(dynamics,num_nodes,pp):
    return "../Results_"+str(dynamics)+"/ER_N%d_p%g_network.dat" % (num_nodes, pp)
