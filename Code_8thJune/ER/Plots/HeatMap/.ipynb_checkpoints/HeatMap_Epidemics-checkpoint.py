...
def savevideo(dynamics, N, pp):
    str_w_vars = "_N%d_p%g.png" % (N, pp)
    os.system("ffmpeg -r 5 -i %02d"+str(str_w_vars)+" -vcodec mpeg4 -y "+str(dynamics)+"_video.mp4")
    return

def fname_mean(dynamics,num_nodes,pp):
    return "../../Results_old/"+str(dynamics)+"/ER_MeanDist_N%d_p%g.dat" % (num_nodes, pp)

def fname_mean_theor(dynamics,num_nodes,pp):
    return "../../Results_old/"+str(dynamics)+"/ER_MeanDist_N%d_p%g_Theoretical.dat" % (num_nodes, pp)

def fname_dij(dynamics, num_nodes, pp, time):
    return "../../Results_old/"+str(dynamics)+"/ER_Dij_N%d_p%g_T%s.dat" % (num_nodes, pp, str('{:.6f}'.format(time)))

def fname_network(dynamics,num_nodes,pp):
    return "../../Results_old/"+str(dynamics)+"/ER_N%d_p%g_network.dat" % (num_nodes, pp)
...
def maximum_value_d(dynamics,N,pp,times_perturbation):
    
    maxv = 0
    for tt in range(1,len(times_perturbation)):
        d_ij = np.loadtxt(fname_dij(dynamics, N, pp, times_perturbation[tt]), comments='#', delimiter=',')
        if np.max(d_ij) > maxv:
            maxv = np.max(d_ij)
    print(maxv)
    return maxv
...
G = nx.read_edgelist(fname_network(dynamics,N,pp))
...
max_value_cb = maximum_value_d(dynamics,N,pp,times_perturbation)
...
#    plt.imshow(sorted_d_ij, cmap=cm.RdYlGn, vmin=0, vmax=max_value_cb)
    ...
    plt.savefig("%02d_N%d_p%g.png" % (tt, N, pp))
    
    plt.close()
#    break

savevideo(dynamics,N,pp)
